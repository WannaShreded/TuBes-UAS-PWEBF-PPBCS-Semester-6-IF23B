...
<head>
    ...
    <style>
        ...
        .orb { animation: floatY 5.5s ease-in-out infinite; }
        .orb-b { animation-delay: .6s; }
        .orb-c { animation-delay: 1.1s; }
        @keyframes floatY {
...
<body class="bg-white text-[#241a52] antialiased">
    ...
    <section class="relative overflow-hidden cosmic-band pb-24">
        ...
        <nav class="relative z-10 flex flex-wrap items-center justify-between gap-5 px-6 pt-9 sm:px-10 lg:px-14">
            <a href="{{ url('/') }}" class="flex items-center gap-2.5 font-display text-lg font-bold tracking-wide text-white">
        ...
        <div class="relative z-10 mt-24">
            <svg viewBox="0 0 1400 120" preserveAspectRatio="none" class="block h-auto w-full -mb-px">
                <path fill="#ffffff" d="M0,80 C240,140 420,10 700,45 C980,80 1160,10 1400,55 L1400,120 L0,120 Z"></path>
            </svg>
        </div>
    </section>
    ...
    <section class="bg-white px-6 pb-24 pt-2 sm:px-10 lg:px-14">
        ...
        <div class="mx-auto mt-14 grid max-w-6xl gap-7 sm:grid-cols-2 lg:grid-cols-3">
            <div class="rounded-[22px] border border-[#ece7fb] bg-[#f4f1fb] p-8 transition hover:-translate-y-1.5 hover:shadow-xl hover:shadow-[#5b1fb8]/10">
                <div class="mb-5 flex h-13 w-13 items-center justify-center rounded-2xl bg-gradient-to-br from-[#22d3ee] to-[#7c1fd6] font-display text-lg font-bold text-white" style="width:52px;height:52px;">
                    01
                </div>
    ...
    <footer class="bg-[#140b3d] px-6 py-12 text-center text-sm text-white/55 sm:px-10 lg:px-14">
        <div class="mb-3 flex items-center justify-center gap-2 font-display font-bold tracking-wide text-white">
            <span class="relative inline-block h-4.5 w-4.5 rounded-full border-2 border-white" style="width:18px;height:18px;">
                <span class="absolute inset-[3px] rounded-full bg-white"></span>
            </span>
            PAYROLLKU
        </div>
        <p>&copy; {{ date('Y') }} {{ config('app.name', 'PayrollKu') }}. Semua hak dilindungi.</p>
        @if (isset($laravelVersion) || app()->has('laravel'))
            <p class="mt-1 text-white/35">Laravel v{{ Illuminate\Foundation\Application::VERSION ?? app()->version() }}</p>
        @endif
    </footer>
