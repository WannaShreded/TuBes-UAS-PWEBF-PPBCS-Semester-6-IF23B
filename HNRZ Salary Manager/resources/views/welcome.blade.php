...
<head>
    ...
    <title>{{ config('app.name', 'PayrollKu') }} — Sistem Penggajian Karyawan</title>
...
<body class="bg-white text-[#241a52] antialiased">
        ...
        <nav class="relative z-10 flex flex-wrap items-center justify-between gap-5 px-6 pt-9 sm:px-10 lg:px-14">
            ...
            @if (Route::has('login'))
                <div class="flex items-center gap-4 font-display text-sm font-semibold tracking-wide">
                    ...
                    @else
                        <a href="{{ route('login') }}" class="text-white/85 transition hover:text-white">
                            Masuk
                        </a>
        ...
        <div class="relative z-10 mx-auto max-w-2xl px-6 pt-16 text-center sm:pt-20">
            ...
            <div class="mt-9 flex flex-wrap justify-center gap-4">
                ...
                @else
                    ...
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}"
                            class="rounded-full border-2 border-white/70 px-9 py-4 font-display text-sm font-bold uppercase tracking-[0.14em] text-white transition hover:bg-white hover:text-[#5b1fb8]">
                            Daftar Sekarang
                        </a>
                    @endif
                @endauth
    ...
    <footer class="bg-[#140b3d] px-6 py-12 text-center text-sm text-white/55 sm:px-10 lg:px-14">
        ...
        @if (isset($laravelVersion) || app()->has('laravel'))
            <p class="mt-1 text-white/35">Laravel
                v{{ Illuminate\Foundation\Application::VERSION ?? app()->version() }}</p>
        @endif
    </footer>
